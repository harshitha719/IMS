...
public class IMSImplementation {
	...
	/**
	 * startPoint This is the initial method of the application,which provided
	 * various options of Menu to users to choose by and proceed ahead
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * readInputFromUser This method is the trigger point of all the main menu
	 * items.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * addRecordmenu This method calls the sub category of menu 4 items and their
	...
	private void addRecordmenu() throws IOException, FileNotFoundException {
		...
		// initialize bufferReader to read console input
		brReader = new BufferedReader(new InputStreamReader(System.in));

		IMSManagerMenu4 manager = new IMSManagerMenu4();
		boolean updateFlag = false;

		// preprocessor call every time menu 4 is loaded. It creates a backup in memory
		// and writes to a file before exiting the menu.
		StringBuffer buffer = manager.changeRecordPreProcessor(this.getDataFile());

		String mainMenuContd = "Y";

		// loop to check whether user wants to continue in menu 4 or exit.
		while (mainMenuContd.equalsIgnoreCase("Y")) {
			...
			// loop to check if user wants to continue in the same sub-menu selection.
			while (submenuContd.equalsIgnoreCase("Y")) {

				switch (selection) {
				// add new record in inventory data file
				case "4.1":
				...
				// delete record from inventory data file
				case "4.2":
					System.out.println(
				...
				// update record in inventory data file
				case "4.3":
				...
				// default choice for invalid selection
				default:
					submenuContd = "N";
			...
			if (mainMenuContd.equalsIgnoreCase("Y")) {
				System.out
						.println("Do you want to continue in menu 4 or return to main menu? Continue -  Y, Return - N");
				mainMenuContd = brReader.readLine();
			}
		}
		// logic to create backup file in case any updates were made. This is created
		// using the initial backup loaded in the memory via preprocessor.
		if (updateFlag) {
			manager.createBackup(buffer);
			clearscr();
			System.out.println(
	...
	/**
	 * searchProduct This method calls the sub category of menu 2 items and their
	 * respective functionality.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * showInventoryMenu This method calls the sub category of menu 3 items and
	...
	public void showInventoryMenu() throws FileNotFoundException, IOException {
		...
		while (choice != "3.6") {
			...
			// get user input
			choice = sc.nextLine();
			// Switch statement for getting inputs from user and calling showInventoryResult
			// class functions based on the user inputs
			switch (choice) {
			case "3.1":
			...
			case "3.2":
				...
				columnNumber = 3;
				// function call to show data for the manufacturer entered by the user, passing
				// file name, manufacturer name and column number
